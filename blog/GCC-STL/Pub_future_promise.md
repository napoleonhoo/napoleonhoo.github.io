...
struct __future_base {
    ...
    public:
        ...
        static void _S_check(const shared_ptr<_Tp>& __p) {
            if (!static_cast<bool>(__p)) __throw_future_error((int)future_errc::no_state);
        }
    ...
    template <typename _BoundFn, typename _Res = decltype(std::declval<_BoundFn&>()())>
...
- `_Result`结构体，继承自`_Result_base`。
  - 成员变量：`__gnu_cxx::__aligned_buffer<_Res> _M_storage;`，数据存储的地方。`_M_set`函数即是设置此变量。
  - 成员变量：`bool _M_initialized;`，表示数据是否被初始化。
  - `struct _Deleter::operator()`函数：调用类内的`_M_destroy`函数。
  - `_M_destroy`函数：销毁this。
  - 不允许拷贝。
- `_Result_alloc`结构体，继承自`_Result`，多了一个allocator，进而利用它来重写了`_M_destroy`函数。
- `_S_allocate_result`函数：为result（`_Result_alloc`、`_Result`）分配空间，返回的是相应类型的`unique_ptr`。
- 类`_State_baseV2`，亦作`_State_base`：
  - `_M_complete_async`函数：等待异步函数完成。
  - `_M_is_deferred_future`函数：返回state是否对应于一个deferred函数。
  - 枚举类型`_Status`：分两种未就绪（not ready）、就绪（ready）。
  - 成员变量：`_Ptr_type _M_result;`存储结果的位置；`__atomic_futex_unsigned<> _M_status;`保存status（关于本类中用到的futex的内容参见备注2.1），`atomic_flag _M_retrieved = ATOMIC_FLAG_INIT;`、`once_flag _M_once;`，用于多线程同步。
  - 构造函数：`_M_result`设为空，`_M_status`设为not ready。
  - 不允许拷贝。
  - `wait`函数：wait直到`_M_status`等于`__Status::ready`，返回`_M_result`。
  - `wait_for`函数：如果`_M_status`等于`__Status::ready`，返回`future_status::ready`；等待一段时间（`__rel`），直到`_M_status`等于`__Status::ready`，返回`future_status::ready`；否则，返回`future_status::timeout`。
  - `wait_until`函数：逻辑和上面的函数类似，区别就是wait for和wait until函数的区别。
  - `_M_do_set`函数：调用传入的函数指针，得到返回值`__res`，`__did_set`设置为true，并将`__res`与`_M_result`交换。这个函数是结合`call_once`来调用的，`call_once`后面的函数只执行一次。
  - `_M_set_result`函数：用传入的函数指针`__res`和`__did_set`调用`_M_do_set`函数（`call_once`调用）。如果设置成功，将`__Status::ready`赋值给`_M_status`，并通知所有。
  - 结构体`_Make_ready`，继承自`__at_thread_exit_elt`。（相关参见备注2.2）
    - 成员变量`_M_shared_state`，是一个指向`_State_baseV2`类型的`weak_ptr`。
    - `_S_run`函数：先对`_M_shared_state`上锁，将ready赋值给此state的`_M_status`，并notify all。
    - `_S_set`函数：将`_S_run`赋值给`_M_cb`，并用this作为参数调用`__at_thread_exit`。（将会在thread退出之前调用`_S_run`。
  - `_M_set_delayed_result`函数：逻辑和`_M_set_result`函数类似，只是赋值status为ready的操作放在了线程退出的时候。
  - `_M_break_promise`函数：当相应的state被抛弃的时候，调用这个函数。
  - `_M_set_retrieved_flag`函数：当state第一次被传到一个future时，调用这个函数。
  - 结构体`_Setter`：用来设置promise的值。有两个模板参数，`_Res`：promise的类型；`_Arg`用来构造`_Res`的参数。
    - 成员变量：`promise<_Res>* _M_promise`，要set的promise；`_Arg* _M_arg`，要构造promise的值的参数。
    - `operator()`函数：用`_M_Arg`构造`_M_promise->_M_storage`。并将其移动返回。
    - 特化形式主要在第二个参数上，可以是普通的`_Arg&`（左值）、`_Res&&`（右值）、`void`（空值）、`__exception_ptr_tag`设置一个异常。
  - `__setter`函数：返回一个用传入的参数构造的`_Setter`结构体的对象。
  - `_S_check`函数：一般在`__setter`函数调用，检查传入的promise的future是否有效。

#### `__future_base::_Result_base`对引用类型的偏特化形式和对void的特化形式
...
`__basic_future`实现了关于`future`和`shared_future`的共享的部分。

- 主要的成员变量是一个`shared_ptr<_State_base> `类型（`__state_type`）的`_M_state`，大多数函数的操作主要都是对其调用。
- 不允许拷贝。
- `wait`、`wait_for`、`wait_until`函数，都是对`_M_state`调用相应的函数。
- `_M_get_result`函数：对`_M_state`调用`wait`函数，然后将其得到的返回值返回。
- 从`shared_future`中拷贝构造：拷贝传入的参数的`_M_state`。
- 从`shared_future`中移动构造：移动传入的参数的`_M_state`。
- 从`future`中拷贝构造：拷贝传入的参数的`_M_state`。
- 默认构造函数：调用`_M_state`的默认构造函数。
...
class future : public __basic_future<_Res> {
    ...
    static_assert(is_destructible<_Res>{}, "result type must be destructible");
...
- 不允许拷贝。
- 带有`__state_type`构造函数：用其来构造`_Base_type`。
- 默认构造函数：默认构造`_Base_type`。
- 移动构造函数：用传入的`future`构造`_Base_type`。
- 移动赋值函数：用传入的`future`移动构造一个新的`future`，并和this交换。
- `get`函数：对this进行reset，并返回值。
- `share`函数：移动this来构造一个`shared_future`。

#### `future`对引用类型的偏特化和对void类型的特化
...
inline shared_future<void> future<void>::share() noexcept { return shared_future<void>(std::move(*this)); }
```

区别主要在`get`函数的设计上，根据不同的类型有不同的返回值。
...
和`future`的区别主要在于，`shared_future`是允许拷贝的。拷贝函数的内容主要是用传入的另一个`shared_future`对象来构造`_Base_type`，即`__basic_future<_Res> `。
...
public:
    ...
    promise(allocator_arg_t, const _Allocator& __a)
        : _M_future(std::allocate_shared<_State>(__a)), _M_storage(__future_base::_S_allocate_result<_Res>(__a)) {}
    ...
    // Setting the result
    void set_value(const _Res& __r) { _M_future->_M_set_result(_State::__setter(this, __r)); }

    void set_value(_Res&& __r) { _M_future->_M_set_result(_State::__setter(this, std::move(__r))); }

    void set_exception(exception_ptr __p) { _M_future->_M_set_result(_State::__setter(__p, this)); }

    void set_value_at_thread_exit(const _Res& __r) {
        _M_future->_M_set_delayed_result(_State::__setter(this, __r), _M_future);
    }

    void set_value_at_thread_exit(_Res&& __r) {
        _M_future->_M_set_delayed_result(_State::__setter(this, std::move(__r)), _M_future);
    }

    void set_exception_at_thread_exit(exception_ptr __p) {
        _M_future->_M_set_delayed_result(_State::__setter(__p, this), _M_future);
    }
};
...
- 主要的两个成员变量：`_M_future`，注意，这个“future”并不是上文谈到的`future`，而是一个指向`__future_base::_State_base`的共享指针；`_M_storage`是一个指向结果类型的指针。
- 默认构造函数、移动构造、移动赋值函数都是对这两个成员变量的操作。
- 不允许拷贝构造、赋值。
- `swap`函数：分别交换这两个成员变量。
- `get_future`函数：用`_M_future`构造一个`future`对象并返回。
- `set_value`函数：调用的是`_M_future->_M_set_result`。
- `set_value_at_thread_exit`函数：调用`_M_future->_M_set_delayed_result`函数。

#### 对`promise`的引用类型偏特化版本和对void的特化版本
...
public:
    ...
    // Setting the result
    void set_value(_Res& __r) { _M_future->_M_set_result(_State::__setter(this, __r)); }
...
public:
    ...
    template <typename _Allocator>
    promise(allocator_arg_t, const _Allocator& __a)
        : _M_future(std::allocate_shared<_State>(__a)), _M_storage(__future_base::_S_allocate_result<void>(__a)) {}
    ...
    // Setting the result
    void set_value() { _M_future->_M_set_result(_State::__setter(this)); }

    void set_exception(exception_ptr __p) { _M_future->_M_set_result(_State::__setter(__p, this)); }

    void set_value_at_thread_exit() { _M_future->_M_set_delayed_result(_State::__setter(this), _M_future); }
...
区别主要在于根据不同的类型，有不同的`get_future`、`set_value`、`set_value_at_thread_exit`的调用方式。

#### `__future_base::_Task_setter`
...
struct __future_base::_Task_setter {
    ...
    _Ptr_type operator()() const {
        ...
        __catch(...) { (*_M_result)->_M_error = current_exception(); }
...
struct __future_base::_Task_state_base<_Res(_Args...)> : __future_base::_State_base {
    ...
    virtual void _M_run_delayed(_Args&&... __args, weak_ptr<_State_base>) = 0;

    virtual shared_ptr<_Task_state_base> _M_reset() = 0;
...
private:
    virtual void _M_run(_Args&&... __args) {
        auto __boundfn = [&]() -> _Res {
            return std::__invoke_r<_Res>(_M_impl._M_fn, std::forward<_Args>(__args)...);
        };
        this->_M_set_result(_S_task_setter(this->_M_result, __boundfn));
    }

    virtual void _M_run_delayed(_Args&&... __args, weak_ptr<_State_base> __self) {
        auto __boundfn = [&]() -> _Res {
            return std::__invoke_r<_Res>(_M_impl._M_fn, std::forward<_Args>(__args)...);
        };
    ...
    virtual shared_ptr<_Task_state_base<_Res(_Args...)> > _M_reset();
...
static shared_ptr<__future_base::_Task_state_base<_Signature> > __create_task_state(_Fn && __fn,
...
__future_base::_Task_state<_Fn, _Alloc, _Res(_Args...)>::_M_reset() {
    return __create_task_state<_Res(_Args...)>(std::move(_M_impl._M_fn), static_cast<_Alloc&>(_M_impl));
}
```

`__Task_state_base`是`_Task_state`的基类。

- `_M_run`函数：调用task，且设置state为ready。
- `_M_run_delayed`函数：调用task，并且在线程退出的时候设置state为ready。
- `_M_reset`函数：重置。

`_Task_state`会在之后的`packaged_task`中有主要用途。

- `_Impl`结构体：主要成员变量是一个函数类型：`_Fn _M_fn`，它的构造函数的主要作用就是构造此成员变量。
- 主要成员变量，即`_Impl`类型的`_M_impl`。
- 构造函数：主要作用就是一个`_M_impl`。
- `_M_run`函数：利用传入的参数调用`_M_impl._M_fn`，并调用`__future_base::_State_base::_M_set_result`。
- `_M_run_delayed`函数：利用传入的参数调用`_M_impl._M_fn`，并调用`__future_base::_State_base::_M_set_delayed_result`。
- `_M_reset`函数：利用传入的函数等构造一个新的`_Task_state_base`。

`__create_task_state`函数：利用传入的参数构造一个指向`_Task_state`的`shared_ptr`。
...
class packaged_task<_Res(_ArgTypes...)> {
    ...
    using __not_same = typename enable_if<!is_same<packaged_task, _Fn2>::value>::type;
...
public:
    ...
    explicit packaged_task(_Fn&& __fn)
        : _M_state(__create_task_state<_Res(_ArgTypes...)>(std::forward<_Fn>(__fn))) {}
...
- 主要成员变量是一个指向`_Task_state_base`的`shared_ptr`：`_M_state`。
- 对传入fn的构造函数：调用`__create_task_state`来创建一个新的`_M_state`。
- 析构函数：在`_M_state`有效，且不是unique的时候（即共享指针被一个以上共享，亦即use count大于1的情况）。
- 不允许拷贝构造、赋值。
- `swap`函数：交换二者的`_M_state`。
- 移动构造函数：调用`swap`函数。
- 移动赋值函数：用传入的参数调用移动构造函数，再调用`swap`函数。
- `get_future`函数：用`_M_state`构造一个`future`。
- `operator()`函数：调用`_M_state->_M_run`，核心是执行task。
- `make_ready_at_thread_exit`函数：调用`_M_state->_M_run_delayed`。
- `reset`函数：创建一个新的`packaged_task`对象，并赋值`_M_state`。（这样的话，这个临时的对象会在这个函数退出的时候析构，从而正确地析构原`_M_state`）并调用`_M_state->_M_reset()`。

#### `__future_base::_Deferred_state`
...
public:
    ...
    explicit _Deferred_state(_Args&&... __args)
        : _M_result(new _Result<_Res>()), _M_fn{{std::forward<_Args>(__args)...}} {}
...
`_Deferred_state`是`async`函数创建的一种共享状态，持有一个deferred函数，和它的结果。

`_M_compelete_async`函数：运行deferred函数。

#### `__future_base::_Async_state_impl`

``` cpp
// Common functionality hoisted out of the _Async_state_impl template.
class __future_base::_Async_state_commonV2 : public __future_base::_State_base {
protected:
...
public:
    template <typename... _Args>
    explicit _Async_state_impl(_Args&&... __args)
        : _M_result(new _Result<_Res>()), _M_fn{{std::forward<_Args>(__args)...}} {
...
private:
    void _M_run() {
        ...
        __catch(const __cxxabiv1::__forced_unwind&) {
            ...
            if (static_cast<bool>(_M_result)) this->_M_break_promise(std::move(_M_result));
...
类`_Async_state_impl`是由`async`函数创建的共享状态，创建一个新的线程来运行特定的函数并且让共享状态ready。而类`_Async_state_commonV2`是它的一个直接基类。

类`_Async_state_commonV2`：

- 主要的成员变量：`thread _M_thread`，是一个线程。
- `_M_join`函数：对`_M_thread`调用`join`函数，且只调用一次。（用`std::call_once`保证）
- `_M_complete_async`函数：调用`_M_join`函数。

类`_Async_state_impl`：

- 主要成员变量：`_M_fn`：相关的函数。`_M_result`：函数运行的结果。
- 构造函数：默认构造一个`_M_result`，利用传入的参数构造一个`_M_fn`，并将`_M_thread`绑定到`_M_run`函数上。
- 析构函数：调用`_M_thread.join()`。
- `_M_run`函数：调用`_M_set_result`来调用`_M_fn`设置`_M_result`。
...
/// async
template <typename _Fn, typename... _Args>
_GLIBCXX_NODISCARD future<__async_result_of<_Fn, _Args...> > async(launch __policy, _Fn && __fn,
                                                                   _Args && ... __args) {
    ...
    if ((__policy & launch::async) == launch::async) {
        __try {
            __state = std::make_shared<_As>(std::forward<_Fn>(__fn), std::forward<_Args>(__args)...);
        }
#if __cpp_exceptions
    ...
    if (!__state) {
        __state = std::make_shared<_Ds>(std::forward<_Fn>(__fn), std::forward<_Args>(__args)...);
    }
...
template <typename _Fn, typename... _Args>
_GLIBCXX_NODISCARD inline future<__async_result_of<_Fn, _Args...> > async(_Fn && __fn, _Args && ... __args) {
    return std::async(launch::async | launch::deferred, std::forward<_Fn>(__fn), std::forward<_Args>(__args)...);
}
```

第一种`async`函数：当launch policy与`launch::async`相与为`launch::async`时，尝试将`__state`指向一个`__future_base::_Async_state_impl`；当相与不为`launch::async`时，或者上一步的尝试失败时，将`__state`指向一个`__future_base::_Deferred_state`。返回一个由`__state`构造的`future`对象。

第二种`async`函数：还是调用第一种`async`函数。但传入的policy是`launch::async | launch::deferred`。（这种情况意味着：`__policy & launch::async = true`）。

## 2 备注

### 2.1 `__atomic_futex_unsigned`相关操作
...
``` cpp
_GLIBCXX_ALWAYS_INLINE bool _M_load_when_equal_for(unsigned __val, memory_order __mo,
                                                           const chrono::duration<_Rep, _Period>& __rtime) ;
```

wait for一段时间（`__ritime`），直到值等于`__val`。
...
第一个成员变量是`__at_thread_exit_elt* _M_next`，相关的数据结构组成了一个链表。第二个成员变量是一个函数指针`_M_cb`。